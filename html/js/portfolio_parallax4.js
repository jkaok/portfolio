...
/* portfolio4 */
    ...
    $(window).scroll(function(){
        ...
        if( wScroll >= $img.eq(3).offset().top - $(window).height()/3 ){
            $img.eq(3).addClass("show");
        }
        if( wScroll >= $img.eq(4).offset().top - $(window).height()/3 ){
            $img.eq(4).addClass("show");
        }
        if( wScroll >= $img.eq(5).offset().top - $(window).height()/3 ){
            $img.eq(5).addClass("show");
        }
        if( wScroll >= $img.eq(6).offset().top - $(window).height()/3 ){
            $img.eq(6).addClass("show");
        }
        if( wScroll >= $img.eq(7).offset().top - $(window).height()/3 ){
            $img.eq(7).addClass("show");
        }
        if( wScroll >= $img.eq(8).offset().top - $(window).height()/3 ){
            $img.eq(8).addClass("show");
        }
        if( wScroll >= $img.eq(9).offset().top - $(window).height()/3 ){
